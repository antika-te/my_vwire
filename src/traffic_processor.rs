...
impl TrafficProcessor {
    ...
    /// 
    /// # Returns
    /// * `Option<Vec<u8>>` - Some(響應數據) 或 None (需要轉發)
    pub async fn process_http_request(
        ...
        if is_ad {
            ...
            Some(HttpParser::build_204_response())
        } else {
            // 3b. 正常內容 - 需要轉發
            self.stats.requests_allowed += 1;
            debug!("  ✓ 放行：{}{}", request.host, request.path);
            
            None // 返回 None 表示需要轉發到後端
        }
    }

    /// 處理完整的 TCP 流
    /// 
    /// 處理客戶端發送的完整 HTTP 請求並返回響應
    pub async fn handle_client(
        ...
        // 嘗試解析為 HTTP 請求
        if let Some(response) = self.process_http_request(client_data, filter_engine).await {
            // 已攔截，返回自構造的響應
            Ok(Some(response))
        } else {
            // 需要轉發到真實後端
...
mod tests {
    ...
    async fn test_process_advertisement_request() {
        ...
        let response = processor.process_http_request(
            &request[..],
            filter_engine,
        ).await;
        
        assert!(response.is_some());
        assert!(response.unwrap().starts_with(b"HTTP/1.1 204"));
    }
    ...
    async fn test_process_normal_request() {
        ...
        assert!(response.is_none()); // 正常內容需要轉發
    }
}
